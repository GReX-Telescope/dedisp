...
dedisp_error update_scrunch_list(dedisp_plan plan) {
  ...
  dedisp_error error = generate_scrunch_list(
      &plan->scrunch_list[0], plan->dm_count, plan->dt, &plan->dm_list[0],
      plan->nchans, plan->f0, plan->df, plan->pulse_width, plan->scrunch_tol);
...
// Public functions
// ----------------
dedisp_error dedisp_create_plan(dedisp_plan *plan_, dedisp_size nchans,
...
dedisp_error dedisp_generate_dm_list(dedisp_plan plan, dedisp_float dm_start,
  ...
  plan->dm_list.clear();
  generate_dm_list(plan->dm_list, dm_start, dm_end, plan->dt, ti, plan->f0,
                   plan->df, plan->nchans, tol);
...
dedisp_error dedisp_execute_guru(const dedisp_plan plan, dedisp_size nsamps,
  ...
  if (out_nbits != 8 && out_nbits != 16 && out_nbits != 32) {
  ...
  */
  ...
  dedisp_size nsamps_padded_gulp_max =
      div_round_up(nsamps_computed_gulp_max, DEDISP_SAMPS_PER_THREAD) *
  ...
  try {
    d_intermediate_buf.resize(nsamps_padded_gulp_max * sb_count * nom_dm_count);
  } catch (...) {
    throw_error(DEDISP_MEM_ALLOC_FAILED);
  }
  dedisp_word *d_intermediate =
      thrust::raw_pointer_cast(&d_intermediate_buf[0]);
  ...
  for (dedisp_size gulp_samp_idx = 0; gulp_samp_idx < nsamps_computed;
    ...
    dedisp_size nsamps_padded_gulp =
        div_round_up(nsamps_computed_gulp, DEDISP_SAMPS_PER_THREAD) *
            DEDISP_SAMPS_PER_THREAD +
        plan->max_delay;
    ...
    if (plan->scrunching_enabled) {
      ...
      for (dedisp_size s = 2; s <= max_scrunch; s *= 2) {
        ...
        scrunch_x2(&d_unpacked[scrunch_in_offset], nsamps_padded_gulp / (s / 2),
    ...
    /* // Consistency checks
       if( (nom_dm_count-1)*dm_stride + (batch_size-1)*batch_dm_stride >=
       dm_count ) { throw std::runtime_error("DM STRIDES ARE INCONSISTENT");
       }
       if( (sb_size-1)*chan_stride + (batch_size-1)*batch_chan_stride >=
       plan->nchans ) { throw std::runtime_error("CHAN STRIDES ARE
       INCONSISTENT");
       }
    */

    // Both steps
    if (!dedisperse(d_transposed, nsamps_padded_gulp, nsamps_computed_gulp,
                    in_nbits, sb_size, chan_stride,
                    thrust::raw_pointer_cast(&plan->d_dm_list[first_dm_idx]),
                    nom_dm_count, dm_stride, (dedisp_byte *)d_intermediate,
                    ostride,
                    32, // out_nbits,
                    batch_size, batch_in_stride, batch_dm_stride,
    ...
    /* // Consistency checks
       ...
       if( (sb_count-1)*chan_stride + (batch_size-1)*batch_chan_stride >=
       plan->nchans ) { throw std::runtime_error("CHAN STRIDES ARE
       INCONSISTENT");
       }
    */

    if (!dedisperse(d_intermediate, nsamps_padded_gulp, nsamps_computed_gulp,
                    32, // in_nbits,
                    sb_count, chan_stride,
                    thrust::raw_pointer_cast(&plan->d_dm_list[first_dm_idx]),
                    dm_size, dm_stride, d_out, ostride, out_nbits, batch_size,
                    batch_in_stride, batch_dm_stride, batch_chan_stride,
                    batch_out_stride)) {
      throw_error(DEDISP_INTERNAL_GPU_ERROR);
    }
    ...
    if (plan->scrunching_enabled) {
      ...
      for (dedisp_size s = 0; s < dm_count; ++s) {
        ...
        if (s + 1 == dm_count || plan->scrunch_list[s + 1] != cur_scrunch) {
          ...
          if (!dedisperse( //&d_transposed[scrunch_offset],
                  &d_unpacked[scrunch_offset], nsamps_padded_gulp / cur_scrunch,
                  nsamps_computed_gulp / cur_scrunch,
                  unpacked_in_nbits, // in_nbits,
                  plan->nchans, 1, d_scrunched_dm_list_ptr,
      ...
      if (!dedisperse( // d_transposed,
              ...
              plan->nchans, 1,
              thrust::raw_pointer_cast(&plan->d_dm_list[first_dm_idx]),
              dm_count, 1, d_out, out_stride_gulp_samples, out_nbits, 1, 0, 0,
              0, 0)) {
        throw_error(DEDISP_INTERNAL_GPU_ERROR);
      }
    }
...
dedisp_error dedisp_execute_adv(const dedisp_plan plan, dedisp_size nsamps,
  ...
  return dedisp_execute_guru(plan, nsamps, in, in_nbits, in_stride, out,
                             out_nbits, out_stride, first_dm_idx, dm_count,
                             flags);
}
