 ...
 * DEDISP_HOST_POINTERS: Instruct the function that the given pointers point to
 * memory on the host.\n DEDISP_DEVICE_POINTERS: Instruct the function that the
 * given pointers point to memory on the device.\n DEDISP_WAIT: Instruct the
 * function to wait until all device operations are complete before returning.\n
 * DEDISP_ASYNC: Instruct the function to return before all device operations
 * are complete.
 */
 ...
 * DEDISP_MEM_COPY_FAILED: A memory copy failed. This is often due to one of the
 * arrays passed to dedisp_execute being too small.\n
 * DEDISP_NCHANS_EXCEEDS_LIMIT: The number of channels exceeds the internal
 * limit. The current limit is 8192.\n DEDISP_INVALID_PLAN: The given plan is
 * NULL.\n DEDISP_INVALID_POINTER: A pointer is invalid, possibly NULL.\n
 * DEDISP_INVALID_STRIDE: A stride value is less than the corresponding
 * dimension's size.\n DEDISP_NO_DM_LIST_SET: No DM list has yet been set using
 * either \ref dedisp_set_dm_list or \ref dedisp_generate_dm_list.\n
 * DEDISP_TOO_FEW_NSAMPS: The number of time samples is less than the maximum
 * dedispersion delay.\n DEDISP_INVALID_FLAG_COMBINATION: Some of the given
 * flags are incompatible.\n DEDISP_UNSUPPORTED_IN_NBITS: The given \p in_nbits
 * value is not supported. See \ref dedisp_execute for supported values.\n
 * DEDISP_UNSUPPORTED_OUT_NBITS: The given \p out_nbits value is not supported.
 * See \ref dedisp_execute for supported values.\n DEDISP_INVALID_DEVICE_INDEX:
 * The given device index does not correspond to a device in the system.\n
 * DEDISP_DEVICE_ALREADY_SET: The device has already been set and cannot be
 * changed. See \ref dedisp_set_device for more info.\n DEDISP_PRIOR_GPU_ERROR:
 * There was an existing GPU error prior to calling the function.\n
 * DEDISP_INTERNAL_GPU_ERROR: An unexpected GPU error has occurred within the
 * library. Please contact the authors if you get this error.\n
 * DEDISP_UNKNOWN_ERROR: An unexpected error has occurred. Please contact the
 * authors if you get this error.
 */
 ...
 *  \p DEDISP_DEVICE_ALREADY_SET, \p DEDISP_UNKNOWN_ERROR, \p
 * DEDISP_PRIOR_GPU_ERROR \note If this function is not called, the
 * system-defined default device will be used. \note This function must be
 * called <b>before any other dedisp library calls.</b> \note This function can
 * only be called once.
 */
 ...
 *  \param gulp_size The new internal gulp size (arbitrary units, default:
 * 131072) \return One of the following error codes: \n \p DEDISP_INVALID_PLAN,
 * \p DEDISP_NO_ERROR \note If this function is not called, the default gulp
 * size (131072) is used.
 */
 ...
 *
 *  \param plan Plan object to set gulp size of
 *  \return The internal gulp size used by the library (arbitrary units,
 * default: 131072)
 */
 ...
 *     1=>include channel.
 *  \return One of the following error codes: \n
 *  \p DEDISP_INVALID_PLAN, \p DEDISP_MEM_COPY_FAILED, \p
 * DEDISP_PRIOR_GPU_ERROR, \p DEDISP_NO_ERROR \note \p killmask must be an array
 * on the host, i.e., it must not be a device array \note Passing \p
 * killmask=NULL sets a blank killmask, i.e., no channels are ignored.
 */
 ...
 *  \p DEDISP_MEM_ALLOC_FAILED, \p DEDISP_MEM_COPY_FAILED, \p
 * DEDISP_PRIOR_GPU_ERROR \note \p dm_list must be an array on the host, i.e.,
 * it must not be a device array \note One of either this function or \p
 * dedisp_generate_dm_list must be called prior to calling dedisp_execute.
 */
 ...
 *  \return One of the following error codes: \n
 *    \p DEDISP_NO_ERROR, \p DEDISP_INVALID_PLAN, \p DEDISP_INVALID_POINTER,
 *    \p DEDISP_INVALID_STRIDE, \p DEDISP_NO_DM_LIST_SET, \p
 * DEDISP_TOO_FEW_NSAMPS, \p DEDISP_INVALID_FLAG_COMBINATION, \p
 * DEDISP_UNSUPPORTED_IN_NBITS, \p DEDISP_UNSUPPORTED_OUT_NBITS, \p
 * DEDISP_INTERNAL_GPU_ERROR, \p DEDISP_PRIOR_GPU_ERROR \note This function
 * assumes \p in and \p out are unpadded arrays. To process arrays containing
 * padding, use \ref dedisp_execute_adv. \note This function assumes one wishes
 * to compute all DMs in the DM list. To compute only a subset of the DM list,
 * use \ref dedisp_execute_guru.
 */
 ...
 * DEDISP_UNSUPPORTED_IN_NBITS, \p DEDISP_UNSUPPORTED_OUT_NBITS, \p
 * DEDISP_INTERNAL_GPU_ERROR, \p DEDISP_PRIOR_GPU_ERROR \note This function
 * assumes one wishes to compute all DMs in the DM list. To compute only a
 * subset of the DM list, use \ref dedisp_execute_guru.
 */
dedisp_error dedisp_execute_adv(const dedisp_plan plan, dedisp_size nsamps,
                                ...
                                unsigned flags);
/*! \p dedisp_execute_guru executes a plan to dedisperse the given array of
 * data. \warning This function is experimental and may contain bugs. \bug This
 * function cannot be used in conjunction with adaptive time resolution.
 *
 *  \param plan The plan to execute.
 *  \param nsamps The length in samples of each input time series.
 *  \param in Pointer to an array containing a time series of length \p nsamps
 ...
 *    Must be >= (\p nsamps - \p max_delay) * \p out_nbits/8, where \p max_delay
 *      is the value given by \p dedisp_get_max_delay(plan).
 *  \param first_dm_idx The (0-based) index at which to start within the DM
 * list. \param dm_count The number of DMs to compute from the DM list. \param
 * flags Configuration flags, interpreted as follows: \n 0 or
 * DEDISP_USE_DEFAULT:        Use the default configuration. \n
 *    DEDISP_HOST_POINTERS (default): The pointers \p in and \p out point to
 *      arrays on the host. \n
 *    DEDISP_DEVICE_POINTERS:         The pointers \p in and \p out point to
 *      arrays on the device. \n
